...
description: 在 X-Border chat 里生成高转化抖音/TikTok 带货短视频。不是裸喂模型——按强 Hook 九宫格方法论:先写强停滑点分镜脚本、产品保真锁定、用 generateImage 出分镜图、再用 generateSeedanceVideo 出 Seedance 2.0 视频(9:16、原生音频)。零 key,经 X-Border 中转。用于「用这张商品图出个带货视频/做条抖音带货视频/强Hook短视频/九宫格出片」这类请求,也用于「给 Temu/Amazon/Noon 等电商平台出主图/详情页视频」。按电商平台(Temu/Noon/Amazon/Shein/Mercado/N11)切换语言/合规/比例/卖点,见 references/platform-profiles.md。
---

# 抖音带货短视频(chat 版 · 强 Hook 方法论)

把「产品图 + 商品信息」做成一条**高转化**的 9:16(或按电商平台比例)带货短视频。核心不是把图丢给模型,而是走一套方法论:**强 Hook 分镜脚本 → 产品保真 → 分镜图 → Seedance 出片**。全程用 X-Border 内置工具,用户零 key。
...
## 平台/引擎参数(内部前置解析 · 交互流程不变)

> **交互流程与现状完全一致**——两阶段(先确认中文脚本再出图/出片)、三条链路、引擎选择(Seedance/Kling)都不变。平台与模型**只改变具体参数与内容**(比例/语言/合规/字幕/卖点/选角),不改变你和用户怎么交互。没提平台时,和现在一模一样。

选链路前,内部读 `references/platform-profiles.md`,产出【本次运行参数】块贯穿全流程:
- 用户明说平台(Temu/Noon/Amazon/Shein/Mercado/N11)→ 用该平台档默认;提到跨境/电商但没指明哪家 → 问一次;**完全没提平台 → 静默走默认档(9:16/强Hook/无字幕/欧美女性),交互与产出跟现在一样(零回归)**。
- genre 默认:货架电商→`listing-demo`,内容电商(默认档)→`social-ad`;用户要投社媒/联盟 → 切 `social-ad`。
- 按优先级(用户 > 平台档 > 全局)解析出【本次运行参数】(比例/genre/语言/选角/字幕/audio_mode/时长/合规/心智)。**后续出脚本、出图、写视频提示词、出片全部引用它,不各自重解析,不留"按档案"悬空指针。**
- 参数随**引擎**落地:Seedance/generateImage 用 `scale`+`aspectRatio`(仅 `1:1/16:9/9:16`);Kling 无比例参数,起始图按运行参数比例出。引擎选择本身不变。
...
- **合规自检(出图前 + 出片前)**:对照【本次运行参数.合规红线】逐条扫——尤其 listing-demo/Amazon:价格/促销/倒计时文字、第三方 logo/水印、绝对化用语、站外 URL、badge 仿冒。命中 → 修正或不出片,不硬出。
...
- 生成中:说明已提交,用对应结果工具查询同一任务。每轮对话最多自动查询 3 次,不得紧循环;仍未完成就把 taskToken 给用户稍后再查。用户再次要求继续时,下一轮仍最多查询 3 次。
